...
class ProxyServer:
    def __init__(self, config: dict):
        ...
        if self.mode == "google_fronting":
            self.mode = "domain_fronting"
            config["mode"] = self.mode
        if self.mode == "apps_script":
            self.mode = "vercel_edge"
            config["mode"] = self.mode
        self.fronter = DomainFronter(config)
        self.mitm = None
        self._cache = ResponseCache(max_mb=50)
        self._relay_host = str(config.get("worker_host", "")).lower().strip(".")
        self._relay_ips = self._resolve_host_ips(self._relay_host)

        # Persistent HTTP tunnel cache for fronting modes
        ...
        if self.mode == "vercel_edge":
            try:
                from mitm import MITMCertManager
                self.mitm = MITMCertManager()
            except ImportError:
                log.error("vercel_edge mode requires 'cryptography' package.")
                log.error("Run: pip install cryptography")
                raise SystemExit(1)

    @staticmethod
    def _resolve_host_ips(host: str) -> set[str]:
        """Resolve a hostname to IP strings. Returns empty set on failure."""
        if not host:
            return set()
        try:
    ...
    def _is_relay_target(self, host: str) -> bool:
        """True if host matches relay hostname or one of its resolved IPs."""
        h = host.lower().strip(".")
        if not h:
            return False
        if self._relay_host and h == self._relay_host:
            return True
        if self._is_ip_literal(h) and h in self._relay_ips:
            return True
        return False
    ...
    async def _do_connect(self, target: str, reader, writer):
        ...
        if self.mode == "vercel_edge":
            if self._is_relay_target(host):
                # Prevent recursive loop: requests to the relay endpoint must
                # never be relayed back through itself.
                log.info("Bypass relay target → direct tunnel %s:%d", host, port)
                await self._do_direct_tunnel(host, port, reader, writer, connect_ip=host)
                return
    ...
    # Optional built-in suffixes for automatic SNI rewrite. Keep empty by
    # default; users can set explicit overrides via `hosts` in config.
    ...
    def _sni_rewrite_ip(self, host: str) -> str | None:
        ...
        Order of precedence:
          ...
          2. Built-in `_SNI_REWRITE_SUFFIXES` → mapped to fronting connect IP.
        """
    ...
    async def _do_direct_tunnel(self, host: str, port: int,
        ...
        Defaults to the configured fronting connect IP.
        """
    ...
    async def _do_sni_rewrite_tunnel(self, host: str, port: int, reader, writer,
        ...
        using SNI=front_domain.

        The ISP only sees the configured front-domain SNI in the outgoing handshake,
    ...
    # ── MITM CONNECT (vercel_edge mode) ───────────────────────────

    async def _do_mitm_connect(self, host: str, port: int, reader, writer):
        """Intercept TLS, decrypt HTTP, and relay through Vercel endpoint."""
    ...
    async def _do_http(self, header_block: bytes, reader, writer):
        ...
        if self.mode == "vercel_edge":
            # Parse request and relay through Vercel endpoint
            ...
            if origin and response:
                response = self._inject_cors_headers(response, origin)
        elif self.mode in ("custom_domain", "domain_fronting"):
