...
class DomainFronter:
    def __init__(self, config: dict):
        ...
        if mode == "custom_domain":
            ...
            self.connect_host = config.get("front_ip") or config.get("front_domain")
            self.sni_host = config.get("front_domain", "vercel.com")
            self.http_host = config["worker_host"]
            # Multi-endpoint round-robin for higher throughput.
            # Use `relay_paths` or a single `relay_path`.
            paths = config.get("relay_paths")
            if not paths:
                single = config.get("relay_path", "/api/api")
                paths = [single]
            self._relay_paths = []
            for p in paths:
                if not p:
                    continue
                p = p if str(p).startswith("/") else f"/{p}"
                self._relay_paths.append(p)
            if not self._relay_paths:
                self._relay_paths = ["/api/api"]
            self._relay_idx = 0
        else:
        ...
        self.enable_h2 = bool(config.get("enable_h2", True))
        ...
        self._batch_enabled = bool(config.get("enable_batch", True))
        ...
        self._h2 = None
        if mode == "vercel_edge" and self.enable_h2:
            try:
    ...
    def _next_relay_path(self) -> str:
        """Round-robin across configured relay endpoint paths."""
        p = self._relay_paths[self._relay_idx % len(self._relay_paths)]
        self._relay_idx += 1
        return p
    ...
    async def _h2_connect_and_warm(self):
        """Connect H2, pre-warm the Vercel relay, start keepalive."""
        await self._h2_connect()
        if self._h2 and self._h2.is_connected:
            asyncio.create_task(self._prewarm_relay())
            asyncio.create_task(self._keepalive_loop())

    async def _prewarm_relay(self):
        """Pre-warm the Vercel relay endpoint."""
        payload = json.dumps(
            {"m": "HEAD", "u": "http://example.com/", "k": self.auth_key}
        ).encode()
        hdrs = {"content-type": "application/json"}
        path = self._next_relay_path()
        try:
            ...
            if "s" in data or "e" in data:
                log.info("Vercel relay pre-warmed in %.0fms", dt)
        except Exception as e:
            log.debug("Pre-warm failed: %s", e)

    async def _keepalive_loop(self):
        """Send periodic pings to keep relay warm + H2 connection alive."""
        while True:
            try:
                ...
                # Relay keepalive — warm the function/container path
    ...
    # ── Vercel relay (vercel_edge mode) ───────────────────────────

    async def relay(self, method: str, url: str,
                    headers: dict, body: bytes = b"") -> bytes:
        """Relay an HTTP request through the Vercel endpoint.
    ...
    async def relay_parallel(self, method: str, url: str,
        ...
        Since each relay call has fixed overhead regardless of payload size,
        we use:
          - 256 KB chunks (safe for proxying through function responses)
    ...
    def _build_payload(self, method, url, headers, body):
        ...
        if headers:
            # Strip Accept-Encoding to avoid compressed-body mismatches
            # between relay and local parser.
    ...
    async def _batch_submit(self, payload: dict) -> bytes:
        ...
        # If batching is disabled, go direct
        ...
        if len(batch) == 1:
            ...
            try:
                ...
                log.warning("Batch relay failed, disabling batch mode. "
                            "Check relay batch support. Error: %s", e)
    ...
    async def _relay_batch(self, payloads: list[dict]) -> list[bytes]:
        """Send multiple requests in one POST using relay batch endpoint."""
        batch_payload = {
            "k": self.auth_key,
            "q": payloads,
        }
    ...
    async def _read_http_response(self, reader: asyncio.StreamReader):
        ...
        # Auto-decompress gzip if present
    ...
    def _parse_relay_response(self, body: bytes) -> bytes:
        """Parse relay JSON and reconstruct an HTTP response."""
    ...
    def _parse_relay_json(self, data: dict) -> bytes:
        ...
        for k, v in resp_headers.items():
            if k.lower() in skip:
                continue
            # Relay may return multi-valued headers (e.g. Set-Cookie) as a
    ...
    def _split_set_cookie(blob: str) -> list[str]:
        ...
        Some relay stacks join multiple Set-Cookie values with ", ",
